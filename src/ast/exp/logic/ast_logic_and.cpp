...
void ast_logic_and::codegen_l() {
    const sem_value &v = child_l->get_value();
    if (v.known) {
        if (!v.value.boo) {
            s_value = {true, {.boo = false}};
        ...
        r_bb = llvm::BasicBlock::Create(llvm_context, "and_r", func);
        cont_bb = llvm::BasicBlock::Create(llvm_context, "and_cont", func);

        ir_builder.CreateCondBr(child_l->get_llvm_value(), r_bb, cont_bb);
        ir_builder.SetInsertPoint(r_bb);
    }
}

void ast_logic_and::codegen_r() {
    ...
    if (child_l->get_value().known) {
        ...
        llvm_value = ir_builder.CreateLoad(llvm_tmp, "load_and_tmp");
    }
}

void ast_logic_and::explain_impl(std::string &res, int indent) const {
    binop_explain_impl("and", res, indent);
}
