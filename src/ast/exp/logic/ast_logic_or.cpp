...
void ast_logic_or::do_logic() {
    s_value = sem::do_logic_or(child_l->get_value(), child_r->get_value());
}

void ast_logic_or::codegen() {
    if (s_value.known) {
        llvm_value = gen::get_llvm_const(s_type, s_value);
    } else {
        llvm_value = gen::gen_logic_or(child_l->get_llvm_value(), child_r->get_llvm_value()
        );
    }
}
