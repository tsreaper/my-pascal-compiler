...
ast_arith_sub::ast_arith_sub(ast_value_node *child_l, ast_value_node *child_r) : ast_binop(child_l, child_r) {}

bool ast_arith_sub::semantics_op() {
    try {
        s_type = sem::assert_can_do_sub(conv_type_l, conv_type_r);
    ...
    if (child_l->get_value().known && child_r->get_value().known) {
        ...
        s_value = sem::do_arith_sub(conv_type_l, conv_type_r, l_val, r_val);
    }
    return true;
}
...
void ast_arith_sub::codegen() {
    if (s_value.known) {
        ...
        llvm_value = gen::gen_arith_sub(GEN_PARAMS);
    }
}

void ast_arith_sub::explain_impl(std::string &res, int indent) const {
    binop_explain_impl("sub", res, indent);
}
