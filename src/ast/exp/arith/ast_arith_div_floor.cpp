...
ast_arith_div_floor::ast_arith_div_floor(ast_value_node *child_l, ast_value_node *child_r) : ast_binop(child_l, child_r) {}

bool ast_arith_div_floor::semantics_op() {
    try {
        s_type = sem::assert_can_do_div_floor(conv_type_l, conv_type_r);
        if (child_r->get_value().known && child_r->get_value().value.num == 0) {
            throw sem_exception("semantics error, cannot divide 0");
        }
    ...
    if (child_l->get_value().known && child_r->get_value().known) {
        ...
        s_value = sem::do_arith_div_floor(conv_type_l, conv_type_r, l_val, r_val);
    }
    return true;
}
...
void ast_arith_div_floor::codegen() {
    if (s_value.known) {
        ...
        llvm_value = gen::gen_arith_div_floor(GEN_PARAMS_NO_TYPE);
    }
}

void ast_arith_div_floor::explain_impl(std::string &res, int indent) const {
    binop_explain_impl("div_floor", res, indent);
}
