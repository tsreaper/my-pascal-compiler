...
bool ast_cmp_ne::semantics_op() {
    ...
    if (child_l->get_value().known && child_r->get_value().known) {
        ...
        s_value = sem::do_cmp_ne(conv_type_l, conv_type_r, l_val, r_val);
    }
    return true;
}
...
void ast_cmp_ne::codegen() {
    if (s_value.known) {
        ...
        if (conv_type_l.tg == type_group::STR) {
            llvm_value = gen::gen_str_ne(GEN_PARAMS_NO_TYPE);
        } else {
            llvm_value = gen::gen_cmp_ne(GEN_PARAMS);
        }
    }
}

void ast_cmp_ne::explain_impl(std::string &res, int indent) const {
    binop_explain_impl("ne", res, indent);
}
